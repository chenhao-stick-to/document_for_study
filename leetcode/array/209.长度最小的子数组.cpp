...
public:
    int minSubArrayLen(int target, vector<int>& nums) {
        ...
        if (nums_sum < target) {
            ...
            int result_count = nums.size();
            while (low < high) {
                cout<<low<<" "<<high<<endl;
                if (nums[low] < nums[high]) {
                less:
                    int temp = nums_sum - nums[low];
                    if (temp > target) {
                        low++;
                        result_count--;
                        nums_sum = temp;
                ...
                } else if (nums[high] == nums[low] && high - low > 1) {
                    int i = low + 1;
                    int j = high - 1;
                    while (i < j) {
                        if (nums[i] < nums[j]) {
...
int main(){
    ...
    int target =697439;
    int x=so.minSubArrayLen(target,nums);
    cout<<x<<endl;
}