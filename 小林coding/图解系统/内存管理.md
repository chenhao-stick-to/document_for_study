...
对于短时间大量的数据到内存，例如接收的网络数据。为了避免kswapd线程异步回收来不及，直接到WATER_MIN之下造成直接内存回收阻塞进程。我们需要提高MIN到LOW的间距，扩大kswapd的活动范围。
**sysctl 来动态调整 watermark_scale_factor 参数，内核会动态重新计算水位线之间的间距，使得 WMARK_MIN 与 WMARK_LOW 之间留有足够的缓冲余地，使得 kswapd 能够有时间回收足够的内存，从而解决直接内存回收导致的性能抖动问题。**
## 物理内存中的冷热页
**热页指加载进入CPU高速缓存的物理内存页，冷页就是还未加载进入CPU的高速缓存中的物理内存页。**
...
```c
struct zone {
    struct per_cpu_pageset	pageset[NR_CPUS];//NR_CPUS表示系统的最大CPU数
}
struct per_cpu_pageset {//per_cpu_pageset管理该CPU的缓存的冷热页
...
```
## 内核如何描述物理内存页
内核为每一个物理内存区域分配了一个伙伴系统，用于管理该物理内存区域下所有物理内存页面的分配和释放。
struct page结构在内核中几乎每个部分都会用到这个结构，所以内核对该结构的一个字段，一个比特位都格外珍惜。而对page不同的场景会用到不同的字段，也会用不到某些字段，==所以page结构体很多使用unio联合体的地方来节约物理内存==
```c
struct page {
...
页高速缓存在内核中的结构体就是这个 struct address_space。它被文件的 inode 所持有。如下图：
![image-20240304150328568](内存管理.assets/image-20240304150328568.png)
```c
struct page {
    // 如果 page 为文件页的话，低位为0，指向 page 所在的 page cache
    // 如果 page 为匿名页的话，低位为1，指向其对应虚拟地址空间的匿名映射区 anon_vma
...
```
page是文件页，mapping指针最低位为0.指向页高速缓存address_space.page_t index字段表示该内存页page在页高速缓存的index索引，也表示该内存页中的文件数据在文件内部的偏移offset，偏移单位page size。
page 是一个匿名页的话，那么 mapping 指针的最低位会被设置为 1 ， 指向该匿名页在进程虚拟内存空间中的匿名映射区域 struct anon_vma 结构（每个匿名页对应唯一的 anon_vma 结构），用于物理内存到虚拟内存的反向映射。
### 匿名页的反向映射
即从物理页寻找被映射到哪些进程的虚拟地址空间中，当回收物理页时，需要找到物理页被映射到了哪些进程的虚拟地址空间中，并断开映射。
有了反向映射机制，可从物理内存页找到所有进程映射的虚拟地址空间VMA，再从VMA中取消该映射。
```c
struct vm_area_struct {  //如下是vma_area_struct与匿名页反向映射相关的字段属性。
    struct list_head anon_vma_chain;
    struct anon_vma *anon_vma;   
}
```
父进程fork子进程的时候，父子进程具有相同的虚拟内存空间；当子进程申请内核的内存时，内核会为这块内存初始化一段虚拟内存区域vma_area_struct，并不会分配真正的物理内存.真的访问该虚拟内存的时候再缺页中断再去分配物理内存（创建属于自己的anon_vma和anon_vma_chain）。
如下图，page的mapping指针指向匿名页的anon_vma产生关联,anno_vma_chain则是关联了匿名页和vma_area_struct。
![image-20240304153834567](内存管理.assets/image-20240304153834567.png)
一个匿名页可对应多个进程的vma_area_struct;即一个anon_vma可对应多个anon_vma_chain。
```c
struct anon_vma {
    ...
    struct rb_root rb_root; /* Interval tree of private "related" vmas 这颗红黑树管理了与该anon_vma相联的所有anon_vma_chain*/
};
...
struct anon_vma_chain {
    ...
    struct anon_vma *anon_vma;//对应的anon_vma
    // 指向 vm_area_struct 中的 anon_vma_chain 列表，指向同一个vma的所有匿名页的列表
    struct list_head same_vma;   
    // anon_vma 管理的红黑树中该 anon_vma_chain 对应的红黑树节点
    struct rb_node rb;   //再anon_vma红黑树中的节点      
};
```
如下图：
![image-20240304154843467](内存管理.assets/image-20240304154843467.png)
```c
struct vm_area_struct {  //一个vma也可以对应多个匿名页
...
```
![image-20240304155419114](内存管理.assets/image-20240304155419114.png)
## 内存页回收相关属性
内核回收内存的时候列表回收优先级：inactive 链表尾部 > inactive 链表头部 > active 链表尾部 > active 链表头部。
内核将页面使用频率这个重要的考量因素加入进来，其工作原理：
- 内存紧张，inactive链表尾部是访问频率最低且最少访问的优先被换出。内核从active尾部开始扫描，将一些页面降级到inactive的头部，而尾部的则被置换。
- 对文件页，第一次访问，放到inactive头部，继续则升到active尾部，再访问则放到头部。否则随着新文件页进入会被逐渐降级。
- 对匿名页，第一次读取就是active的尾部，因为匿名页的换入换出成本很大，文件也是脏页才需要回写。再次访问放在active的头部。
为什么会把 active 链表和 inactive 链表分成两类，一类是匿名页，一类是文件页？
可设置内核的参数，cat /proc/sys/vm/swappiness。为0则表示只置换文件页，不置换匿名页。
将其分开是为了区分的对待，因为文件页的swap比匿名页的更高效，所以分开以作区分。
除以上四种链表，还有一种链表。进程可通过mlock()等系统调用将内存页锁定在内存中，保证内存页不会被换出。故内核又将这些被锁定的页面单独拎出来放在一个独立的链表中。
```c
struct page {
   struct list_head lru;//标识放在哪个链表上。
   atomic_t _refcount;
}
```
### 物理内存页属性和状态的标志位flag
...
这两个信息都存储在page的flags的字段中。高位为page的定位信息，低位标识页的属性位，如下图：
![image-20240304164453619](内存管理.assets/image-20240304164453619.png)
如下有页的标志位说明：
...
PG_slab 表示该物理内存页属于 slab 分配器所管理的一部分。基于特定内核数据结构的内存管理数据结构。
...
//如上用于进程阻塞直到页面解锁，针对页面各种状态，一些有一些内核函数可以阻塞等待直到页面的状态改变。
```
### 复合页compound_page相关属性
linux管理内存的最小单位是page。每个page描述4K大小的物理内存。一些场景，用户希望使用巨型大页。巨型大页就是通过两个或者多个物理上连续的内存页 page 组装成的一个比普通内存页 page 更大的页。
巨型页可减少缺页中断发生，同时TLB的缓存项减少，故减少了缓存miss的概率，提升性能。另外就是如一个内存占用大的进程（redis）fork新进程时需要拷贝父进程资源，包括父进程的页表。由于巨型页面，页表项减少，拷贝性能提升！
巨型页如何管理？复合页的诞生！如下图：
![image-20240304172155988](内存管理.assets/image-20240304172155988.png)
```c
    struct page {      
...
```
![image-20240304172558273](内存管理.assets/image-20240304172558273.png)
### Slab 对象池相关属性
内核对内存页的分配有两种方式：
- 一种是一页一页的分配使用，这种以页为单位的分配方式内核会向相应内存区域 zone 里的伙伴系统申请以及释放。
- 另一种方式就是只分配小块的内存，不需要一下分配一页的内存。slab分配器。
```c
struct page {//如下图是page里面关于slab分配器的一些相关信息
