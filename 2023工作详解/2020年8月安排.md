...
（4）git的详细使用/gitlab的使用学习/clang-format格式化代码格式的是操作流程和设置/clang-tidy进行代码的规范/c++设置launch.json进行调试，gdb调试使用/linux终端命令的开发使用/CI流水线的使用/cmake的系统使用以及构建风格/各种平台，谷歌，vscode，终端等的快捷操作键的学习/gdb的使用/vscode扩展插件的熟练使用。/openapi的使用/搭建github的博客系统(长期
